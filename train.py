    ...
    with tf.Graph().as_default():
        ...
        for epoch in range(MAX_EPOCH):
            ...
            if max_accuracy_eval < mean_acc_eval:
                max_accuracy_eval = mean_acc_eval
                saver.save(sess, os.path.join(LOG_DIR, "model_best_eval_avg_class_acc.ckpt"))
            if max_accuracy_eval < accuracy_eval:
                max_accuracy_eval = accuracy_eval
                saver.save(sess, os.path.join(LOG_DIR, "model_best_acc_eval.ckpt"))
...
def eval_one_epoch(sess, ops, test_writer):
    ...
    return (np.mean(np.array(total_correct_class) / np.array(total_seen_class, dtype=np.float))), (
