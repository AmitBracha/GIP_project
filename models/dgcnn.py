...
def get_model(point_cloud, is_training, bn_decay=None):
    ...
    net = tf_util.conv2d(edge_feature, 1024, [1, 1],
...
def get_mode_net4_full_momentum(point_cloud, is_training, bn_decay=None):
    ...
    edge_feature = tf_util.get_momentum_feature(net, k=64)
...
def get_model_old_6(point_cloud, is_training, bn_decay=None):
    ...
    edge_feature = tf_util.get_zeros_edge_feature(point_cloud, k=k)

    with tf.variable_scope('transform_net1') as sc:
        transform = input_transform_net(edge_feature, is_training, bn_decay, K=3)

    point_cloud_transformed = tf.matmul(point_cloud, transform)
    edge_feature = tf_util.get_zeros_edge_feature(point_cloud_transformed, k=k)
...
def get_model_old_5(point_cloud, is_training, bn_decay=None):
    ...
    edge_feature = tf_util.get_feature_double(point_cloud, k=k)

    with tf.variable_scope('transform_net1') as sc:
        transform = input_transform_net(edge_feature, is_training, bn_decay, K=3)

    point_cloud_transformed = tf.matmul(point_cloud, transform)
    edge_feature = tf_util.get_feature_double(point_cloud_transformed, k=k)
...
def get_model_old_2(point_cloud, is_training, bn_decay=None):
    """ Classification PointNet, input is BxNx3, output Bx40 """
    batch_size = point_cloud.get_shape()[0].value
    num_point = point_cloud.get_shape()[1].value
    end_points = {}
    ...
    edge_feature = tf_util.get_momentum_feature(net, k=k)
...
def get_model_old(point_cloud, is_training, bn_decay=None):
    ...
    point_cloud_2_momentum = tf.concat(
        [tf.multiply(point_cloud, point_cloud), tf.multiply(tf.roll(point_cloud, shift=1, axis=-1), point_cloud)],
        axis=1)
    point_cloud = tf.concat([point_cloud, point_cloud_2_momentum], axis=1)
