...
def get_random_edge_feature(point_cloud, layer_index, k=20):
    """Construct edge feature for each point
    Args:
      point_cloud: (batch_size, num_points, 1, num_dims)
      nn_idx: (batch_size, num_points, k)
    ...
    point_cloud_central_shape = point_cloud_central.get_shape().as_list()
    n = point_cloud_central_shape[1] * point_cloud_central_shape[2] * point_cloud_central_shape[3]

    random_feature = tf.multiply(tf.math.sqrt(2 / tf.math.pow(n, layer_index - 1)),
                                 tf.random.normal(point_cloud_central.get_shape()))
...
def get_feature_double(point_cloud, k=20):
    ...
    edge_feature = tf.concat([point_cloud_central, point_cloud_central], axis=-1)
    return edge_feature
...
def get_edge_feature_and_momentum(point_cloud, nn_idx, k=20):
    """Construct edge feature for each point
    Args:
      point_cloud: (batch_size, num_points, 1, num_dims)
      nn_idx: (batch_size, num_points, k)
